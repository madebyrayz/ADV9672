...
<p>In Troika's <i>Ghost Specimen</i>, the artists began with a pressed herbarium sheet of a flower that had been extinct for a century. Pressing keeps one flat aspect of the organism and destroys the other. Asked to reconstruct the whole plant, a generative image model produced the missing face from the statistical regularities of every other specimen it had seen. The result, which joins the original pressed specimen to the generated face, was then exhibited on a wall, where the model's “imperfection” was no longer visible${cite(6)}.</p>
<p>An earlier precedent for this experiment is the visualisation of fluid-dynamics data by electromechanical plotters at Los Alamos in the 1950s and 1960s, as recorded in the laboratory's reports${cite(7)}. Those drawings let scientists study shockwaves and high-velocity collisions, conditions under which solids deform, liquefy or vaporise, without running the physical experiment. A series of drawings was produced from cumulative calculation, so that phenomena and spatial configurations that could not be realised physically could still be pictured and examined.</p>
<p>Both projects work at the boundary between the real and the artificial. When a machine meets incomplete or ambiguous information it is built to produce a plausible reconstruction, which is to say that it hallucinates, and the reconstruction carries the biases of its training data and of its architecture. It is fair to ask, for example, whether the weighting inside a deep network favours some kinds of image over others. Authorship and agency become harder to locate. In this experiment three agents, Holbein, the regression model and myself, negotiate a shared agency across three different perceptions of what is real.</p>
</section>
...
<p>A Python port of Boxer's two <a href="https://idolsofthecave.com/cabinet/anamorphic-ambassadors-may-2012/" target="_blank" rel="noopener">MATLAB scripts</a> returns every published number to within 0.2 mm. The port also shows how little the construction depends on. The horizontal-jaw condition requires only that the jaw line pass through S, which fixes D = ${b.D.toFixed(2)} mm in a single line; the square condition is a ratio, which fixes d = ${b.d.toFixed(2)} mm.</p>
<p>The same transparency shows where the construction rests. The whole result depends on one judgement, that a painted jawbone is a straight line, and Boxer says so himself. The method is exact, but it is exact about a premise that cannot be checked against the panel.</p>
<p>The port also corrected a misunderstanding in this study's own brief rather than anything in Boxer's work. His two transforms are not approximations of one another; they are the same function. Perspective projection at distance R and angle α equals the inverse trapezoid with D = R / sin α and d = R cot α, identically. The 36 mm between his two published points is the orientation of the screen the image is projected onto, not an error. At the millimetre scale, "the" viewing point is already a convention before any model is involved.</p>
...
<p>Perception places the viewer somewhere in a large region of acceptable smears; geometry places them within a few millimetres.</p>
<p>These are different kinds of answer, and the study keeps them apart rather than splitting the difference. This is also the quantitative form of Boxer's objection to the National Gallery's method: adjusting the image until it looks right cannot be more precise than the tolerance of looking, and that tolerance turns out to be very wide.</p>
${F(4, M.phase1.figure,
...
<p>SHARP was given the same photograph. When an image carries no camera data the model assumes a 30 mm lens and reports depth in metres derived from that assumption. There was no lens in 1533, so every metre the model returns rests on a default.</p>
<p>The model did not reconstruct a panel; it reconstructed the room the painting depicts. The floor advances, the curtain recedes, and 623 mm of relief appear across a surface 2095 mm wide that is physically flat. A plane fitted to the image border tilts by twenty degrees, because the bottom border is the depicted floor.</p>
...
<p>The scoring crop cannot be placed where the flat panel puts the skull, because that box comes back almost empty: the model has not placed the skull on the wall. The crop instead follows the projected 3D bounding box of the Gaussians belonging to the skull in the photograph${cite(11)}. At Boxer's O that box is 87 % empty.</p>
<p>Resemblance across the whole grid stays between 0.30 and 0.61, below what the flat panel scores even at the National Gallery's point, and its maximum sits 141 mm from Boxer's O, at (675, 1035, 160). Measured against the disagreement between the two human estimates (13 mm along the wall, 138 mm out from it), that is eight units off along the wall and less than one unit out from it. That number is not a station point in Boxer's sense; it is the best available view of a streak lying on a receding floor.</p>
...
<p>The depth map reads best from O: the model's estimate of where the skull is, stretched across the floor where Holbein stretched the skull. Whether that is an artefact or a diagram is a question the print leaves to the visitor who walks up to it.</p>
...
<p>The geometer's answer is the viewpoint saved in this instrument as the resolved skull: (740.5, 1035, 255.3) mm, R = 1806 mm, 81.9° from the wall normal, where the flat panel scores ${flat} and Boxer's construction reproduces to a hundredth of a millimetre. The model's answer is not a small error; it is an answer to a different question. Given a photograph with no lens it fills in a room, and once there is a room, the floor carries the smear away from the wall on which the construction depends.</p>
<p>This is the useful result, and it runs against the theory it was meant to illustrate. Baudrillard's model does not pretend to see a skull; it produces the symptoms of a scene, and those symptoms are coherent enough to be measured. They were measured, and at a specific coordinate they were found wanting. The hyperreal is supposed to have absorbed its outside. Here the outside held, not because the model is weak but because the object was chosen so that a physical fact remained recoverable.</p>
<p>On this evidence, the precession of simulacra is less a property of models than of situations in which nothing remains to check them, and such situations are arranged rather than given.</p>
<p>Benjamin's aura did not stay in the panel, and it did not pass to the model. It sits in the bridge, the list of assumptions that allow millimetres to be compared with metres. Davis is right that the moment of seeing survives reproduction; here it survives as a coordinate that someone has to argue for, and that argument is the work.</p>
</section>

<section>
<h2><span class="num">11</span>Objections</h2>
<p>Four objections, in descending order of how much weight they carry against the result.</p>
<ul>
...
<li><b>Boxer's premise cannot be tested against the panel.</b> The construction assumes that the painted jawbone is a straight line, and nothing in the object can confirm it. The ground truth is a precise consequence of a reading that cannot itself be checked.</li>
<li><b>The comparison is unfair by design.</b> SHARP was not built to solve anamorphosis; it was built to produce plausible geometry from one photograph, and by that standard it succeeds. The test is not one of competence. It asks what a model does with a question it cannot know it is being asked, and whether the answer arrives marked as a guess. It arrives, instead, in metres.</li>
</ul>
...
<p>Every number in the app belongs to exactly one of these frames; the bridge exists so that they are not confused.</p>
...
<tr><th>Bridge</th><td>The stated conversion between the two, as <i>mm per SHARP unit</i>. It is an assumption, it is recorded in every capture, and it is the number most likely to invalidate a comparison if it is wrong.</td></tr>
</table>
...
<tr><th>az · el · d</th><td>Direction from eye to skull: azimuth from the wall normal, elevation, distance in mm. Grazing views, which the model tends to favour, show high azimuth and negative elevation.</td></tr>
<tr><th>fov</th><td>Field of view in degrees. It sets how much of the smear is in frame, and the 5 mm result in §08 depends on it.</td></tr>
...
<tr><th>Relief</th><td>Depth p95 minus p05 in panel millimetres: how much depth the model has added across a flat board. 623 mm at the 30 mm default.</td></tr>
<tr><th>mm per unit</th><td>The bridge, stated. 1095 at the default lens.</td></tr>
<tr><th>Grid peak · Offset</th><td>Best-scoring position on the resection grid, and its distance from Boxer's O. 141 mm at the default lens, the study's main disagreement.</td></tr>
</table>
</section>
