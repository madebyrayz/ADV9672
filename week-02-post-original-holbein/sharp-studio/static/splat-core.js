...
function createWorker(self) {
    ...
    function processPlyBuffer(inputBuffer) {
        ...
        // Only consider properties belonging to the vertex element (SHARP plys
        // append extra metadata elements after the vertex block).
        const vertexBlock = /element vertex \d+\n([\s\S]*?)(?=\nelement |\nend_header)/.exec(header)[1];
        for (let prop of vertexBlock
            .split("\n")
    ...
    self.onmessage = (e) => {
        if (e.data.ply) {
            ...
            if (e.data.force) forceSort = true;
            throttledSort();
        }
    };
}
